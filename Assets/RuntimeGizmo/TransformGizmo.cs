...
namespace RuntimeGizmos
{
	...
	public class TransformGizmo : MonoBehaviour
	{
		...
		private bool containsUnMessableObject;
		...
		void OnEndContextRendering(ScriptableRenderContext context, Camera[] cameras)
		{
		...
		void SetSpaceAndType()
		{
			...
			else if (Input.GetKeyDown(SetRotateType) && !containsUnMessableObject) transformType = TransformType.Rotate;
			else if (Input.GetKeyDown(SetScaleType) && !containsUnMessableObject) transformType = TransformType.Scale;
		...
		void GetTarget()
		{
			if (nearAxis == Axis.None && Input.GetMouseButtonDown(0))
			{
				...
				Vector3 ray = myCamera.ScreenToWorldPoint(Input.mousePosition);
				Debug.DrawLine(myCamera.transform.position, ray, Color.red, 25f);
				/*if (Physics.Raycast(ray, out RaycastHit hitInfo, 1000))// && hit.transform.gameObject.name == "Flat Plane")
				{
					Transform target = hitInfo.transform;
					Debug.Log(target);
					if (target.tag != "Untouchable")
					{
		...
		// ReSharper disable Unity.PerformanceAnalysis
		public void PassTarget(RaycastHit hitInfo)
		{
			if (nearAxis == Axis.None)
			{
				bool isAdding = Input.GetKey(AddSelection);
		...
		public void RemoveTarget(Transform target, bool addCommand = true)
		{
			if (target != null)
			{
				...
				if (target.gameObject.layer == LayerMask.NameToLayer("Player"))
				{
					...
					containsUnMessableObject = false;
				}
		...
		public void DeleteObject(bool addCommand = true)
		{
			if (!containsUnMessableObject)
			{

				

				var targ = targetRoots.FirstOrDefault();

				if (addCommand) UndoRedoManager.Insert(new DeleteObjectCommand(this, targ.Key));
				;

				GameObject.Destroy(targ.Key.gameObject);
				RemoveTarget(targ.Key, false);
			}
		}
		...
		public void DuplicateObject()
		{
			if (!containsUnMessableObject)
			{
				UndoRedoManager.Insert(new DuplicateCommand(this, null, targetRoots.Keys.ToArray()));
				foreach (var targ in targetRoots)
				{
					Instantiate(targ.Key.gameObject, targ.Key.position, targ.Key.rotation, targ.Key.parent);
				}
			}
		}
